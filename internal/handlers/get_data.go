...
func GetServiceDataAdmin(c echo.Context) error {
	...
	for i := range serverListData {
		...
		for j := range service.Servers {
			...
			discount += serverDiscountMap[serverData.ServerNumber]
...
func GetServiceData(c echo.Context) error {
	...
	for _, service := range services {
		...
		for _, server := range service.Servers {
			...
			// Calculate discounts
			discount := CalculateDiscount(serviceDiscounts, serverDiscounts, userDiscounts, service.Name, server.ServerNumber, userId)
